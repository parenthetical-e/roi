...
def mean_fir(self, window_size):
    ...
    # instance
    ts_bold = nt.timeseries.TimeSeries(self.bold, 
            sampling_interval=self.TR)

    # And another one for the events (the different stimuli):
    ts_trials = ts.timeseries.TimeSeries(
            self.trials, sampling_interval=self.TR)

    # Create a nitime Analyzer instance.
    eva = nt.analysis.EventRelatedAnalyzer(ts_bold, 
            ts_trials, window_size)
    ...
    fir = eva.FIR 
    hrf = fir.mean(0)
        ## hrf if the mean of all conditions in trials
